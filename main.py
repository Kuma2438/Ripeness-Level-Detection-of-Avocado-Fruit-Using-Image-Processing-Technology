...
def print_banner():
    print("=" * 65)
    print(" [+] AVOCADO RIPENESS DETECTION SYSTEM — UNIFIED CONTROL CENTER ")
    print("=" * 65)
...
def run_ml_pipeline():
    ...
    for f in test_files:
        img = cv2.imread(f)
        start = time.time()
        annotated, cat, score, conf = classifier.predict_frame(img)
        times.append((time.time() - start) * 1000)
        print(f"  Image {os.path.basename(f)} -> Class: {cat:8s} | Score: {score:5.1f}% | Conf: {conf:4.0f}% | Latency: {times[-1]:.2f} ms")
...
def run_git_push():
    ...
    if not commit_msg:
        commit_msg = "Update project code and models"
...
def interactive_menu():
 ...
 Select an option to execute:
  ...
  [2] Generate / Refresh Sample Dataset (generate_dataset.py)
  [3] Run ML Model Pipeline & Performance Check
...
def main():
    parser = argparse.ArgumentParser(description="Avocado Ripeness Detection System Unified CLI")
